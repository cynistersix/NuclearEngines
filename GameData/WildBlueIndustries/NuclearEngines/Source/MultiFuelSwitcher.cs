    ...
    public class MultiFuelSwitcher : ExtendedPartModule
    {
        ...
        protected void setup_module()
        {
            try
            {
                ...
                //load the propellant types
                //When we get to this point in flight mode, say, we won't have access to the module's PROPELLANT_TYPE nodes listed in the part's config file.
                //However, we squirreled them away in the _subNodes object when KSP first loaded the part (during application startup).
                if (_propellantTypes == null)
                    _propellantTypes = _subNodes.GetNodes("PROPELLANT_TYPE");
